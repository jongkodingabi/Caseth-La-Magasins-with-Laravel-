    ...
    <div class="w-full max-w-7xl px-4 md:px-5 lg-6 mx-auto relative z-10">
        <div class="grid grid-cols-12">
            <div
                ...
                @if($cartItems->isNotEmpty()) <!-- Menggunakan data dari database -->
                @php
                $totalPrice = 0;
                @endphp
                    <h2 class="font-manrope font-bold text-3xl leading-10 text-black">Shopping Cart</h2>
                    <h2 class="font-manrope font-bold text-xl leading-8 text-gray-600">{{ $totalItems }} Items</h2>
                </div>
    ...
    @foreach ($cartItems as $item)
        <div class="flex flex-col min-[500px]:flex-row min-[500px]:items-center gap-5 py-6 border-b border-gray-200 group">
            <div class="w-full md:max-w-[126px]">
                <img src="{{ asset('asset-views/img/products/' . $item->product->picture) }}" alt=""
                class="mx-auto rounded-xl object-cover">
            </div>
            <div class="grid grid-cols-1 md:grid-cols-4 w-full">
                <div class="md:col-span-2">
                    <div class="flex flex-col max-[500px]:items-center gap-3">
                        <h6 class="font-semibold text-base leading-7 text-black">{{ $item->product->product }}</h6>
                        <h6 class="font-normal text-base leading-7 text-gray-500">{{ $item->product->category }}</h6>
                        <h6 class="font-medium text-base leading-7 text-gray-600 transition-all duration-300 group-hover:text-rose-300">
                ...
                <div class="flex items-center max-[500px]:justify-center h-full max-md:mt-3">
                    <div class="flex items-center justify-center h-full" style="flex-direction: row;">
                        ...
                        <input type="text" class="border-y border-gray-200 outline-none text-gray-900 font-semibold text-lg w-full max-w-[73px] min-w-[60px] text-center bg-transparent" value="{{ $item->quantity }}" readonly>

                        <form action="{{ route('cart.update') }}" method="POST" style="display: inline;">
                            @csrf
                            <input type="hidden" name="id" value="{{ $item->product->id }}">
                            <button type="submit" name="quantity" value="{{ $item->quantity + 1 }}" class="group rounded-r-xl px-5 py-[18px] border border-gray-200 flex items-center justify-center shadow-sm transition-all duration-500 hover:bg-gray-50 hover:border-gray-300 focus-within:outline-gray-300">
                                <svg class="stroke-gray-900 transition-all duration-500 group-hover:stroke-black" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
                ...
                <div class="flex items-center max-[500px]:justify-center md:justify-end max-md:mt-3 h-full">
                    <p class="font-bold text-lg leading-8 text-gray-600 text-center transition-all duration-300 group-hover:text-rose-300">
            ...
            <div
                ...
                <div class="mt-8">
                    <div class="flex items-center justify-between pb-6">
                        ...
                        <p class="font-medium text-lg leading-8 text-black">Rp {{ number_format($totalPrice, 2) }}</p>
                    </div>

                    <form action="{{ route('cart.checkCost') }}" method="POST">
                        @csrf
                        <label for="origin" class="flex items-center mb-1.5 text-gray-600 text-sm font-medium">Pilih Kota Asal
                        </label>
                        <div class="flex pb-6">
                            <div class="relative w-full z-10">
                                <select name="origin" id="origin" class="block w-full h-11 pr-10 pl-4 text-base font-normal
                                shadow-xs text-gray-900 bg-white border border-gray-300 rounded-lg focus:outline-gray-400" required>
                        ...
                        <label for="destination" class="flex items-center mb-1.5 text-gray-600 text-sm font-medium">Pilih Kota Tujuan
                        </label>
                        <div class="flex pb-6">
                            <div class="relative w-full z-10">
                                <select name="destination" id="destination" class="block w-full h-11 pr-10 pl-4 text-base font-normal
                                shadow-xs text-gray-900 bg-white border border-gray-300 rounded-lg focus:outline-gray-400" required>
                        ...
                        <label for="courier" class="flex items-center mb-1.5 text-gray-600 text-sm font-medium">Courier
                        </label>
                        <div class="flex pb-6">
                            <div class="relative w-full z-10">
                                <select name="courier" id="courier" class="block w-full h-11 pr-10 pl-4 text-base font-normal
                                shadow-xs text-gray-900 bg-white border border-gray-300 rounded-lg focus:outline-gray-400" required>
                        ...
                        @if ($cost != '')
                            ...
                            <h2 class="mb-3">
                                <ul>
                                    ...
                                    <li class="font-semibold">Kota Tujuan: <span class="font-light">{{ $cost['destination_details']['city_name'] }}</span></li>
                                    <li class="font-semibold">Berat Paket: <span class="font-light">{{ $cost['query']['weight']}} gram</span></li>
                                </ul>
                            </h2>
                                ...
                                @foreach ($TotalCost['costs'] as $costs)
                                        <div class="mb-3">
                                            ...
                                            @foreach ($costs['cost'] as $value)
                                                <div class="mb-3">
                                                    <label for="value">
                                                        Price: Rp.{{ number_format($value['value'], 2) }} (est: {{ $value['etd'] }} day)
                                                    </label>
                                                </div>
                                                @endforeach
                                            </div>
                        ...
                        <div class="flex items-center justify-between py-8">
                            ...
                            <p class="font-semibold text-xl leading-8 text-rose-300">Rp {{ number_format($totalPrice, 2) }}</p>
                        </div>
                        <button
